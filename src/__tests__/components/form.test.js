...
jest.mock('react-router-dom'); // need this to use Link in test
...
describe('<Form />', () => {
  it('renders the <Form /> with populated data', () => {
    ...
    expect(getByText('Sign In').disabled).toBeTruthy(); // at this point button should be disabled, because no login details have been entered
  ...
  it('renders the <Form /> with an error', () => {
    ...
    expect(queryByText('Sign In').disabled).toBeFalsy(); // should not be disabled, to get an error someone would have typed something in fields
    expect(container.firstChild).toMatchSnapshot();
  });
});